...
**도로 실거리로 전환(2026-09-02, 사용자 지적 반영)**: "출장거리를 실제 도보나 대중교통 기준으로 바꿀 수 있나"는 질문에서, 대상지가 용인·여주·양평·가평·남양주·광주의 수변 녹지·매수토지라 도보·대중교통은 실제 이동 방식과 맞지 않는다고 판단해 **자동차 도로 실거리**로 방향을 확인받았다. `common/road_distance.py`가 OSRM(Open Source Routing Machine) 공개 데모 서버(`router.project-osrm.org`, API 키 불필요, OpenStreetMap 도로망)로 pairwise 주행거리 행렬을 조회한다 — "무상 공개 자료만 쓴다"는 이 프로젝트의 원칙과 같은 선상이라 카카오/네이버 같은 상용 API 대신 골랐다. `module_o/routing.py`는 `Distances` 래퍼로 도로거리를 우선하고, 좌표가 없거나 조회가 실패하면(공개 데모 서버라 상용 SLA가 없다) 예외 없이 직선거리로 폴백한다 — 절대 일부만 도로거리·일부만 직선거리로 섞지 않고, `distance_basis`("driving" | "straight_line")로 어느 쪽인지 항상 표시한다. `pytest module_o/tests/test_routing.py common/tests/test_road_distance.py -v` 통과, 기존 직선거리 테스트 10개는 수정 없이 그대로 통과(하위 호환).

**아직 반영 안 된 것 — 정적 스냅샷과 제안서 수치**: `ui/public/api/route/*.json`(배포본이 읽는 정적 파일)과 제안서에 인쇄된 145.5→122.0km/415.1→252.4km은 여전히 **직선거리 기준 옛 수치**다. `scripts/build_static_api.py`를 다시 돌려 스냅샷을 갱신하면 이 값들이 바뀌므로, 재생성·재배포와 제안서 수치 수정은 사용자 확인 후에 진행한다(§12 TODO로 남겨둠).
