...
> **"코드가 동작한다"와 "실제 성능이 검증됐다"는 다르다.** 예를 들어 Module VERIFY의 `Precision@K` 함수가 테스트를 통과한다는 건 *계산 코드가 맞다*는 뜻이지 *본 시스템의 실제 탐지 정확도가 높다*는 뜻이 아니다. 아래 표는 그 구분을 명시한다.
...
**이건 실패가 아니라 설계가 옳다는 증거다.** 본 시스템의 핵심 명제가 "위성은 확정하지 않는다,
