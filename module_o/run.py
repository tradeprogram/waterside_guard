...
전체 AOI의 Module RISK 결과를 모아 Top-N 우선순위 큐를 생성한다. 우선순위 점수를
...
def run(input: dict) -> dict:
    ...
    for r in risk_results:
        ...
        entry = store.upsert_risk_result(
            site_id,
            inspection_priority_score=r.get("inspection_priority_score"),
            priority_tier=r.get("priority_tier"),
            contributing_factors=r.get("contributing_factors"),
        )
        entries.append(entry)

    entries.sort(key=lambda e: e.get("inspection_priority_score") or 0, reverse=True)
