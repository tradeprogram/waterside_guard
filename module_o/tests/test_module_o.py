    ...
    result = run(
        {
            "week_of": "2026-09-01",
            "risk_results": [
                {"site_id": "A", "inspection_priority_score": 30, "priority_tier": "3순위"},
                {"site_id": "B", "inspection_priority_score": 80, "priority_tier": "1순위"},
                {"site_id": "C", "inspection_priority_score": 55, "priority_tier": "2순위"},
            ],
        }
    )
...
def test_site_without_id_is_skipped_with_warning():
    result = run(
        {
            "week_of": "2026-09-01",
            "risk_results": [{"inspection_priority_score": 50}],
        }
    )
    assert result["status"] == "degraded"
...
def test_inspected_site_shows_as_checked_in_next_queue_run():
    run(
        {
            "week_of": "2026-09-01",
            "risk_results": [{"site_id": "A", "inspection_priority_score": 80, "priority_tier": "1순위"}],
        }
    )
    ...
    result = run(
        {
            "week_of": "2026-09-08",
            "risk_results": [{"site_id": "A", "inspection_priority_score": 40, "priority_tier": "3순위"}],
        }
    )
...
def test_upsert_preserves_inspection_history_across_runs():
    run({"week_of": "W1", "risk_results": [{"site_id": "A", "inspection_priority_score": 80, "priority_tier": "1순위"}]})
    store.record_inspection("A", {"actual_anomaly_found": True})
    run({"week_of": "W2", "risk_results": [{"site_id": "A", "inspection_priority_score": 20, "priority_tier": "정상"}]})
